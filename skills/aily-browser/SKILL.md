...
1. **点点自动检测**：9222 端口是否有带调试端口的 Chrome 在运行
...
- **你的桌面 Chrome 已关闭**：如果桌面 Chrome 正在运行，点点无法启动调试端口（Chrome 限制）
...
# 点点启动 Chrome（自动复制你的 profile，继承登录状态）
...
- **桌面 Chrome 必须关闭**：如果检测到桌面 Chrome 在运行，点点会提示你先关闭
- **Cookie 同步时机**：点点只在启动时复制一次 profile。如果你在桌面 Chrome 里登录了新网站，需要关闭后重新运行点点命令
...
| 没有继承登录状态 | 确保你先关闭了桌面 Chrome，点点才能复制最新的 profile |
