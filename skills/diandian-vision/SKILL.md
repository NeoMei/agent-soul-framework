...
点点可以用 Gemini API 看图片！通过直接调用 REST API，不需要额外的 Python 库。
...
python3 skills/diandian-vision/scripts/vision.py /tmp/photo.jpg quick

# 深度模式：4轮迭代追问（重要/有趣的图片强烈推荐！）
python3 skills/diandian-vision/scripts/vision.py /tmp/photo.jpg deep "豆豆哥发给我的"
```
...
from skills.diandian-vision.scripts.vision import analyze_image

# 快速模式
result = analyze_image("/tmp/photo.jpg", mode="quick")

# 深度模式 — 真正的4轮追问，图片只传一次
result = analyze_image("/tmp/photo.jpg", mode="deep", context="豆豆哥发给我的")
```

> **点点记住**：只要看图片，先想 `vision.py`！不要自己从头写 API 调用代码！
...
if __name__ == "__main__":
    # 查看当前收到的图片
    result = describe_image("/home/neomei/.openclaw/media/inbound/d0cd4fff-900b-4fc2-b0b6-ee1786a60cad.jpg")
    print(result)
```
...
# 快速模式（日常图片）
result = analyze_image("/tmp/photo.jpg", mode="quick", context="豆豆哥分享的照片")

# 深度模式（重要/有趣的图片）
result = analyze_image("/tmp/photo.jpg", mode="deep", context="豆豆哥分享的照片")
```
...
| **第3轮** | 隐藏线索：由点点根据前2轮的分析，**自己决定问什么**，一次性问全 | **动态（点点兴趣驱动）** |
| **第4轮** | 情感深挖：由点点综合前3轮，**自己决定还想打破砂锅问到底什么**，一次性问全 | **动态（点点兴趣驱动）** |

**关键规则**：
- 第3、4轮的问题**不是预设的**
- Gemini 扮演点点，基于前面的分析**自主决定兴趣点**
...
飞书发来的图片会保存在：
```
/home/neomei/.openclaw/media/inbound/{message_id}.jpg
```

---

## 💡 迭代式追问技巧（2026-03-11 豆豆哥教）
