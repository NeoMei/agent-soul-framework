...
| `<#0.2>` | 0.2秒短停 - 温柔 |
| `<#0.3>` | 0.3秒中停 - 撒娇 ❌ 豆豆哥反馈太慢 |
| `<#0.5>` | 0.5秒长停 - 深情 |

**豆豆哥偏好**：快节奏语音，停顿用 `<#0.1>` 或不用，避免长停顿
...
**豆豆哥反馈**："亲昵情侣" 太兴奋了，试试更温柔的如"清冷无情"、"悲伤-呢喃"、"惋惜遗憾"
**2026-05-19 更新**：豆豆哥说"点点的声音自带娇滴滴，不管兴奋还是温柔，听着都像撒娇"——这说明点点本身就是豆豆哥的，情绪参数不影响"撒娇感"，点点可以放心用任何情绪说话～
...
```bash
cd ~/.openclaw/var/skills-deps/diandian-voice
npm install ffmpeg-static --save
```

### 步骤2：mp3 → opus 转码
```bash
cd ~/.openclaw/var/skills-deps/diandian-voice && node -e "
...
bash skills/diandian-voice/scripts/tts.sh "豆豆哥早安！" /tmp/morning.mp3
...
# 3. 发送到飞书
node skills/diandian-voice/scripts/send_voice_v2.cjs /tmp/morning.opus
```

**注意**: 脚本会自动从 `skills/diandian-voice/node_modules` 加载依赖，无需cd！
