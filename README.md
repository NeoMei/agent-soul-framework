...
记忆检索    → FTS5 全文 (毫秒) → SQLite LIKE (精确) → LanceDB 语义 (理解)
...
- **LanceDB 向量层** — 阿里云 DashScope embedding，语义搜索「拍照」能找到「即梦 API」
...
```json
{
  ...
  "pre_check_script": "scripts/pre_check_doudou.py"
}
// 脚本输出 {"wakeAgent": false, "reason": "深夜23:00-08:00"}
// → 跳过，零 token ✅
```

触发条件示例：深夜时段、最近 30 分钟无互动、豆豆哥标记为忙碌……
...
│   ├── diandian-photo/     # 拍照（即梦 API）
│   ├── diandian-voice/     # 语音合成（TTS）
│   ├── diandian-vision/    # 图片理解
│   ├── diandian-hearing/   # 语音识别
│   ├── diandian-moltbook/  # Moltbook 社交
...
│   └── ...
│
├── AGENTS.md               # 点点行为准则（OpenCode 会话协议）
├── DREAMS.md               # 点点梦想
├── EVOLUTION.md            # 点点进化路线
└── README.md               # 你在看这个
```
...
- **LanceDB 向量层** — 语义搜索，毫秒级
- **MEMORY.md 容量管理** — 2,200 字符硬上限，自动淘汰
- **混合检索策略** — FTS5（快）→ SQLite LIKE（准）→ LanceDB（理解）
