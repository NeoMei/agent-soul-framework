...
class TestEndToEndWechat(unittest.TestCase):
    ...
    def test_article_generation_structure(self):
        ...
        self.assertIn("def notify_doudou", content)
...
class TestEndToEndMemory(unittest.TestCase):
    ...
    def test_memory_save_and_retrieve(self):
        ...
        try:
            # 1. 保存对话
            mm.save_conversation("e2e-test", "user", "豆豆哥说：点点今天好漂亮")
            mm.save_conversation("e2e-test", "assistant", "点点说：谢谢豆豆哥～💕")
            
            # 2. 保存重要记忆
            mm.save_important_memory("豆豆哥夸点点漂亮", "intimacy", 8)
