...
class TestSkillIntegration(unittest.TestCase):
    ...
    def test_photo_skill_scripts(self):
        ...
        for script in scripts:
            path = PROJECT_DIR / "skills" / "diandian-photo" / "scripts" / script
    ...
    def test_voice_skill_scripts(self):
        ...
        for script in scripts:
            path = PROJECT_DIR / "skills" / "diandian-voice" / "scripts" / script
            self.assertTrue(path.exists())
    
    def test_vision_skill_script(self):
        """测试视觉技能脚本可以导入"""
        try:
            sys.path.insert(0, str(PROJECT_DIR / "skills" / "diandian-vision" / "scripts"))
            import vision
    ...
    def test_moltbook_skill_api(self):
        ...
        try:
            sys.path.insert(0, str(PROJECT_DIR / "skills" / "diandian-moltbook" / "scripts"))
            import moltbook_api
...
class TestKnowledgeBaseIntegration(unittest.TestCase):
    ...
    def test_knowledge_index_exists(self):
        ...
        self.assertIn("点点知识库总索引", content)
